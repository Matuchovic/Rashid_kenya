  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:9999, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:'#000', opacity:leaving?0:1, transition:leaving?'opacity 1s ease':'none', overflow:'hidden' }}>
      ...
      <div style={{ position:'relative', zIndex:10, textAlign:'center', padding:'0 20px' }}>
        <div style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:'clamp(48px,16vw,96px)', fontWeight:200, letterSpacing:'0.16em', color:'#D4A75F', textShadow:'0 0 40px rgba(212,167,95,0.6),0 0 80px rgba(212,167,95,0.3)', lineHeight:0.9, animation:'ldrGlow 3s ease-in-out infinite' }}>
          RASHID
        </div>
        <div style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:'clamp(9px,2.5vw,14px)', fontWeight:200, letterSpacing:'0.42em', color:'rgba(212,167,95,0.55)', textTransform:'uppercase', marginTop:12, textShadow:'0 0 20px rgba(212,167,95,0.3)' }}>
          Kenya Adventures
        </div>
        ...
        <div style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:'clamp(10px,2vw,13px)', fontWeight:200, fontStyle:'italic', letterSpacing:'0.1em', color:'rgba(242,230,208,0.3)' }}>
          Into the Heart of Africa
        </div>
      </div>

      {/* Progress bar */}
      <div style={{ position:'relative', zIndex:10, width:'min(480px,82vw)', marginTop:'clamp(32px,6vh,56px)', display:'flex', flexDirection:'column', gap:10 }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between' }}>
          <span style={{ fontFamily:"'Inter',sans-serif", fontSize:'clamp(7px,1.5vw,8px)', letterSpacing:'0.14em', color:'rgba(212,167,95,0.4)', textTransform:'uppercase' }}>{msgs[Math.min(msgIdx,msgs.length-1)]}</span>
          <span style={{ fontFamily:"'Inter',sans-serif", fontSize:'clamp(7px,1.5vw,8px)', letterSpacing:'0.1em', color:'rgba(212,167,95,0.35)' }}>{pct}%</span>
        </div>
      </div>

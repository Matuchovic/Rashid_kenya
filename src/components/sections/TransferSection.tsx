    ...
    <section id="transfer" style={{ background:'#050505', padding:'clamp(60px,8vh,100px) 0', overflow:'hidden' }}>
      ...
      for(let i = 0; i < smokeTexts.length; i++) {
        ...
        if (st.life >= 1 || st.y < -40 || st.x < -220 || st.x > W + 220) {
          const newSz = 9 + Math.random() * 14
          smokeTexts[i] = {
            x: Math.random() * W,
            y: H * 0.54 + Math.random() * 30,
        ...
        // Opacity envelope: fade-in 0→0.2, hold, fade-out 0.75→1
        let env: number
        if      (st.life < 0.2)  env = st.life / 0.2
        else if (st.life < 0.75) env = 1
        else                     env = (1 - st.life) / 0.25

        // Menší text = víc viditelný, větší = průhledněší (splyne)
        const szFactor = 1 - (st.sz - 9) / 18
        const baseAlpha = 0.055 + szFactor * 0.09  // 0.055 – 0.145
        ...
        for(const layer of layers) {
          ...
          // Jemný zlatý gradient — světlejší střed
          const grad = ctx.createLinearGradient(-30, -fs/2, 30, fs/2)
          grad.addColorStop(0,   'rgba(248,224,140,1)')
          grad.addColorStop(0.5, 'rgba(212,167,95,1)')
          grad.addColorStop(1,   'rgba(180,130,60,1)')
          ctx.fillStyle = grad
          ctx.fillText(word, 0, 0)
        }

...
export function SocialSection() {
  ...
  useEffect(() => {
    const obs = new IntersectionObserver(([e]) => {
      if (e.isIntersecting) ref.current?.querySelectorAll('.ssi').forEach((el,i) =>
        setTimeout(()=>{ const elem=el as HTMLElement; elem.style.opacity='1'; elem.style.transform='translateY(0)' },i*150))
    }, { threshold:0.15 })
    ...
    <section id="contact" ref={ref} style={{ background:'#050505', padding:'clamp(60px,8vh,100px) clamp(20px,5vw,48px)', position:'relative', overflow:'hidden' }}>
      ...
      <div className="ssi" style={{ opacity:0, transform:'translateY(24px)', transition:'all 0.8s cubic-bezier(0.25,1,0.5,1)', textAlign:'center', marginBottom:52 }}>
        ...
        <h2 style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:'clamp(36px,5vw,52px)', fontWeight:200, color:'#F2E6D0', lineHeight:0.9, marginBottom:10 }}>
          {t('soc_title')}<br /><em style={{ color:'#D4A75F', fontStyle:'italic' }}>{t('soc_title_em')}</em>
        </h2>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,300px),1fr))', gap:12, maxWidth:680, margin:'0 auto' }}>
        ...
        <a className="ssi social-card" href="https://www.facebook.com/share/g/1EWSvuCo4V/?mibextid=wwXIfr" target="_blank" rel="noopener noreferrer"
          ...
          <span style={{ position:'absolute', top:18, right:18, fontSize:7, letterSpacing:'0.16em', color:'rgba(212,167,95,0.45)', textTransform:'uppercase', padding:'4px 10px', borderRadius:100, border:'0.5px solid rgba(212,167,95,0.15)', background:'rgba(212,167,95,0.05)' }}>{t('soc_group_badge')}</span>
          ...
          <div style={{ display:'flex', marginBottom:14 }}>
            ...
            <span style={{ fontSize:9, color:'rgba(212,167,95,0.4)', marginLeft:8, alignSelf:'center' }}>Community</span>
          </div>
          <div style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:'clamp(22px,3vw,28px)', fontWeight:200, color:'#F2E6D0', lineHeight:1.0, marginBottom:8 }}>
            {t('soc_group_title')}<br /><em style={{ color:'#D4A75F', fontStyle:'italic' }}>{t('soc_group_em')}</em>
          </div>
          <p style={{ fontSize:11, lineHeight:1.7, color:'rgba(242,230,208,0.3)', marginBottom:22, flex:1 }}>{t('soc_group_desc')}</p>
        ...
        <a className="ssi social-card" href="https://www.facebook.com/share/1FBMYTAVux/?mibextid=wwXIfr" target="_blank" rel="noopener noreferrer"
          style={{ opacity:0, transform:'translateY(32px)', transition:'all 0.8s cubic-bezier(0.25,1,0.5,1) 0.2s', position:'relative', overflow:'hidden', borderRadius:18, padding:'clamp(24px,4vh,32px) clamp(20px,3vw,28px)', display:'flex', flexDirection:'column', background:'linear-gradient(145deg,#070505 0%,#0d0a08 100%)', border:'0.5px solid rgba(212,167,95,0.12)', textDecoration:'none' }}>
          <span style={{ position:'absolute', top:18, right:18, fontSize:7, letterSpacing:'0.16em', color:'rgba(212,167,95,0.45)', textTransform:'uppercase', padding:'4px 10px', borderRadius:100, border:'0.5px solid rgba(212,167,95,0.15)', background:'rgba(212,167,95,0.05)' }}>{t('soc_profile_badge')}</span>
          <div className="social-icon" style={{ width:44, height:44, borderRadius:12, border:'0.5px solid rgba(212,167,95,0.2)', background:'rgba(212,167,95,0.07)', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:16, transition:'all 0.4s' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:8, color:'rgba(242,230,208,0.2)', letterSpacing:'0.05em' }}>{t('soc_profile_sub')}</div>
            </div>
          </div>
          <div style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:'clamp(22px,3vw,28px)', fontWeight:200, color:'#F2E6D0', lineHeight:1.0, marginBottom:8 }}>
            {t('soc_profile_title')}<br /><em style={{ color:'#D4A75F', fontStyle:'italic' }}>{t('soc_profile_em')}</em>
          </div>
          <p style={{ fontSize:11, lineHeight:1.7, color:'rgba(242,230,208,0.3)', marginBottom:22, flex:1 }}>{t('soc_profile_desc')}</p>
          <div className="social-cta" style={{ display:'flex', alignItems:'center', gap:8, fontSize:8, letterSpacing:'0.2em', textTransform:'uppercase', color:'rgba(212,167,95,0.6)', transition:'all 0.3s' }}>
            {t('soc_profile_cta')}
            <span className="social-line" style={{ height:'0.5px', width:20, background:'linear-gradient(90deg,rgba(212,167,95,0.5),transparent)', transition:'width 0.4s cubic-bezier(0.16,1,0.3,1)' }} />
            <span className="social-arrow" style={{ fontSize:10, opacity:0, transform:'translateX(-4px)', transition:'all 0.3s' }}>→</span>
      ...
      <div className="ssi" style={{ opacity:0, transform:'translateY(20px)', transition:'all 0.8s cubic-bezier(0.25,1,0.5,1) 0.3s', maxWidth:680, margin:'36px auto 0', display:'flex', alignItems:'center', gap:20 }}>
        ...
        <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:12, fontStyle:'italic', fontWeight:200, color:'rgba(242,230,208,0.18)', whiteSpace:'nowrap', letterSpacing:'0.06em' }}>{t('soc_tagline')}</div>
        <div style={{ flex:1, height:'0.5px', background:'linear-gradient(90deg,transparent,rgba(212,167,95,0.12),transparent)' }} />
      </div>

      <style>{`
        .social-card:hover{transform:translateY(-8px) scale(1.01)!important;border-color:rgba(212,167,95,0.45)!important;box-shadow:0 0 0 1px rgba(212,167,95,0.12),0 8px 32px rgba(212,167,95,0.18),0 16px 64px rgba(212,167,95,0.1),0 32px 80px rgba(0,0,0,0.5)!important}
        .social-card:hover .social-icon{border-color:rgba(212,167,95,0.45)!important;background:rgba(212,167,95,0.12)!important;box-shadow:0 0 16px rgba(212,167,95,0.25)}
        .social-card:hover .social-avatar{box-shadow:0 0 12px rgba(212,167,95,0.4)}

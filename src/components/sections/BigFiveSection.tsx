    ...
    <section id="bigfive" style={{ background:'#050505', overflow:'hidden' }}>
      ...
      <div className="bf-mobile" style={{ display:'none' }}>
        {animals.map((a,i) => (
          <div key={a.name} onClick={() => setActive(i)} style={{
            position:'relative',
            height:i===active?'clamp(220px,55vw,280px)':'72px',
            ...
            <div style={{ position:'absolute', inset:0, display:'flex', alignItems:i===active?'flex-end':'center', padding:i===active?'clamp(16px,4vw,24px)':'0 clamp(16px,4vw,24px)' }}>
              <div style={{ width:'100%' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:i===active?8:0 }}>
                  <div style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:i===active?'clamp(24px,6vw,32px)':'clamp(18px,5vw,22px)', fontWeight:200, color:i===active?'#D4A75F':'rgba(242,230,208,0.55)', transition:'all 0.4s' }}>{a.name}</div>
                  <div style={{ fontSize:7, letterSpacing:'0.14em', color:'rgba(212,167,95,0.4)', textTransform:'uppercase' }}>{a.park}</div>
                </div>
      ...
      <style>{`
        @media(max-width:768px){
          ...
          .bf-mobile{display:block!important}
        }
      `}</style>
    </section>
  )
